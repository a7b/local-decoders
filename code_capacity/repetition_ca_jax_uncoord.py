...
def init_state(key, L, p):
    """
    Initialize state: qubits, messages, per-site clocks, step count, RNG key.
    """
    key1, key2 = jax.random.split(key)
    # qubits on edges.
    qubits = jax.random.bernoulli(key1, p, shape=(L,)).astype(jnp.bool_)
    m_grid = jnp.zeros(L, dtype=jnp.bool_)
    c_grid = jnp.zeros(L, dtype=jnp.int32)
    step_count = jnp.array(0, dtype=jnp.int32)
    return (qubits, m_grid, c_grid, step_count, key2)
...
def _single_site_update_step(state):
    """
    Single step of the CA (one site update) using direct indexing.
    state: (qubits, m_grid, c_grid, step_count, key)
    """
    qubits, m_grid, c_grid, step_count, key = state
    L = qubits.shape[0]

    # --- Random Site Selection ---
    key, subkey = jax.random.split(key)
    idx = jax.random.randint(subkey, shape=(), minval=0, maxval=L)

    # Helper for periodic indexing
    def get_at(arr, i):
        return arr[i % L]
        
    def get_s_at(q_arr, i):
        # s[i] = q[i] ^ q[i+1]
        return get_at(q_arr, i) ^ get_at(q_arr, i+1)

    s_idx = get_s_at(qubits, idx)
    s_right = get_s_at(qubits, idx + 1)
    m_idx = get_at(m_grid, idx)
    m_minus_1 = get_at(m_grid, idx - 1)
    c_idx = get_at(c_grid, idx)

    # Algorithm 1 with an independently advancing q=2 clock at this site.
    do_correct_val = s_idx & m_minus_1
    incoming = s_right & m_idx
    growth = (~s_idx) & (~m_idx) & (c_idx == 0) & m_minus_1
    persistence = (~s_idx) & m_idx & m_minus_1
    next_m_val = s_idx | incoming | growth | persistence

    new_m_grid = m_grid.at[idx].set(next_m_val.astype(jnp.bool_))

    # A moving defect flips qubit idx. Only the selected site's messages and
    # clock are updated during this asynchronous step.
    new_qubits = qubits.at[idx].set((qubits[idx] ^ do_correct_val).astype(jnp.bool_))
    new_c_grid = c_grid.at[idx].set((c_idx + 1) % 2)

    return (new_qubits, new_m_grid, new_c_grid, step_count + 1, key)


@jax.jit
def step_sweep(state):
    """
    Perform L single-site updates (one sweep).
    """
    qubits, _, _, _, _ = state
    L = qubits.shape[0]
    
    def scan_body(carry, _):
        next_state = _single_site_update_step(carry)
        return next_state, None
        
    final_state, _ = jax.lax.scan(scan_body, state, None, length=L)
    return final_state


# --- Simulation (Monte Carlo) ---

@partial(jax.jit, static_argnames=['L', 'max_sweeps'])
def run_single_trajectory_while(key, L, p, max_sweeps):
    """
    Run one trajectory using while_loop for early termination.
    Checks convergence after every single-site update.
    Returns 1.0 if logical error, 0.0 otherwise.
    """
    initial_state = init_state(key, L, p)
    
    def cond_fun(state):
        qubits, m_grid, c_grid, step_count, _ = state
        syndrome = calculate_syndrome(qubits)
        # not_cleared = jnp.any(syndrome) | jnp.any(m_grid)
        not_cleared = jnp.any(syndrome)
        # step_count is total single-site updates; max_sweeps remains the public
        # sweep-count argument used by the original script.
        under_step_limit = step_count < (max_sweeps * L)
        return not_cleared & under_step_limit
        
    final_state = jax.lax.while_loop(cond_fun, _single_site_update_step, initial_state)
    
    qubits, m_grid, c_grid, step_count, _ = final_state
...
def run_monte_carlo(L_values, p_values, num_samples, filename="rep_ca_data.npz"):
    ...
    for L in L_values:
        # Literal-update phase cap used by the registered numerical campaign.
        max_sweeps = 64 * L
        # JIT compile the vmapped function specifically for this L
        # vmap over keys
        runner = jax.jit(jax.vmap(partial(run_single_trajectory_while, L=L, max_sweeps=max_sweeps, p=None), in_axes=0))
        ...
        def batch_run(keys, p_val):
             return jax.vmap(lambda k: run_single_trajectory_while(k, L, p_val, max_sweeps))(keys)
...
def save_simulation_gif(L, p, steps, filename="repetition_ca.gif"):
    ...
    def scan_step(carry, _):
        qubits, m, clocks, step_count, done, key = carry
        
        # Calculate next
        # If done, just identity
        
        # Real step
        q_next, m_next, c_next, sc_next, key_next = _single_site_update_step(
            (qubits, m, clocks, step_count, key))
        ...
        c_final = jnp.where(done, clocks, c_next)
        sc_final = jnp.where(done, step_count, sc_next)
        new_carry = (q_final, m_final, c_final, sc_final, new_done, key_next)
...
def simulate_spacetime_data(key, L, p, steps):
    ...
    def scan_step(carry, _):
        qubits, m, clocks, step_count, key = carry
    ...
    final_q, final_m, _, _, _ = final_state
...
if __name__ == "__main__":
    run_monte_carlo([16, 32, 64, 128, 256, 512], [0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.475, 0.5], 1_000_000, filename="rep_ca_data_uncoord.npz")
    plot_results("rep_ca_data_uncoord.npz", "rep_ca_uncoord.png")
